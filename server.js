...
async function getStooqPrice(stooqSym) {
  ...
  if(stooqSym.includes('xau')) {
    const sources = [
      {url:'https://stooq.com/q/l/?s=xauusd.cf&f=sd2t2ohlcv&h&e=csv', parse:'csv'},
      {url:'https://stooq.com/q/l/?s=xauusd&f=sd2t2ohlcv&h&e=csv', parse:'csv'},
      {url:'https://api.metals.live/v1/spot/gold', parse:'json'},
    ];
    for(const src of sources){
      try{
        const res=await fetch(src.url,{timeout:6000});
        const text=await res.text();
        let price;
        if(src.parse==='json'){ const j=JSON.parse(text); price=j.price||j[0]?.price; }
        else{ const p=text.trim().split('\n')[1]?.split(','); price=parseFloat(p?.[6]); }
        if(price&&!isNaN(price)&&price>1000&&price<6000){
          cache.price[key]={v:price,ts:now}; return price;
        }
      }catch(e){continue;}
    }
...
// ============================================================
// FIREBASE ADMIN SDK — Push Notifications V1 API
// ============================================================
...
if (!admin.apps.length) {
  admin.initializeApp({
    credential: admin.credential.cert({
      projectId: 'tradeplus-fe71d',
      clientEmail: 'firebase-adminsdk-fbsvc@tradeplus-fe71d.iam.gserviceaccount.com',
...
async function sendFCMPush(title, body, data = {}) {
  if (!fcmTokens.size) { console.log('No FCM tokens registered'); return; }
    ...
    try {
      await messaging.send({
        ...
        android: {
          ...
          notification: {
            sound: 'default',
            channelId: 'tradeplus_signals',
            priority: 'max',
            defaultVibrateTimings: false,
            vibrateTimingsMillis: [500, 100, 500, 100, 500],
          },
        },
        webpush: {
      ...
      console.log(`✅ FCM push sent to ${token.slice(0,20)}...`);
    } catch(e) {
