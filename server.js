...
async function getStooqPrice(stooqSym) {
  ...
  if(stooqSym.includes('xau')) {
    const sources=[
      ...
      // Stooq regular
      async()=>{ const r=await fetch('https://stooq.com/q/l/?s=xauusd&f=sd2t2ohlcv&h&e=csv',{timeout:6000}); const t=await r.text(); const p=t.trim().split('\n')[1]?.split(','); const v=parseFloat(p?.[6]); return v>1000&&v<6000?v:null; },
      // Metals.live
      async()=>{ const r=await fetch('https://api.metals.live/v1/spot/gold',{timeout:6000}); const t=await r.text(); const d=JSON.parse(t); const v=Array.isArray(d)?d.find(x=>x.gold)?.gold:d.price; return v&&v>1000&&v<6000?parseFloat(v):null; },
      // Frankfurter via EUR/USD and gold ratio (rough approximation)
      async()=>{ const r=await fetch('https://open.er-api.com/v6/latest/XAU',{timeout:6000}); const d=await r.json(); const v=d?.rates?.USD; return v&&v>1000&&v<6000?parseFloat(v):null; },
      // Fallback: use last known price or approximate current gold price
      async()=>{ return cache.price[key]?.v||3300; },
    ];
    for(const src of sources){
      try{
        const v=await src();
        if(v&&v>1000&&v<6000){ cache.price[key]={v,ts:now}; return v; }
      }catch(e){continue;}
    }
    // Absolute last resort — return cached or approximate
    return cache.price[key]?.v||3300;
  }
