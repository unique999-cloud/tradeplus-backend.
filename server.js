...
try {
  ...
  if (!admin.apps.length) {
    ...
    if (privateKey && privateKey.includes('BEGIN PRIVATE KEY')) {
      ...
      console.log('⚠️ FIREBASE_PRIVATE_KEY not set — push notifications disabled');
    }
  }
} catch(e) {
...
async function sendFCMPush(title, body, data = {}) {
  if (!firebaseReady || !admin || !fcmTokens.size) return;
  ...
  for (const token of fcmTokens) {
    try {
      await messaging.send({
        ...
        android: { priority: 'high', notification: { sound: 'default', channelId: 'tradeplus_signals' } },
