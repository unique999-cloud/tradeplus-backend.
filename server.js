...
const PAIRS = [
  ...
  { symbol:'XAU/USD', twelve:'XAU/USD', stooq:'xauusd.cf', dp:2, pip:0.1    },
];
...
const learningDB = {
  ...
  weights: {            // Dynamic indicator weights — updated by learning
    ...
    fib: 1, pivots: 1, momentum: 1, fvg: 1, session: 1,
  },
...
const TTL = {
  ...
  PRICE:30*1000, NEWS:10*60*1000, DXY:5*60*1000,
};

// ============================================================
// STOOQ LIVE PRICE — matches MT5 within 1-2 pips
// ============================================================
async function getStooqPrice(stooqSym) {
  ...
  if(stooqSym.includes('xau')) {
    const sources=[
      async()=>{ const r=await fetch('https://stooq.com/q/l/?s=xauusd.cf&f=sd2t2ohlcv&h&e=csv',{timeout:6000}); const t=await r.text(); const p=t.trim().split('\n')[1]?.split(','); const v=parseFloat(p?.[6]); return v>1000&&v<6000?v:null; },
      async()=>{ const r=await fetch('https://api.metals.live/v1/spot/gold',{timeout:6000}); const d=await r.json(); const v=d.price||d[0]?.price; return v>1000&&v<6000?parseFloat(v):null; },
      async()=>{ const r=await fetch('https://open.er-api.com/v6/latest/XAU',{timeout:6000}); const d=await r.json(); const v=d?.rates?.USD; return v>1000?parseFloat(v):null; },
    ];
    for(const src of sources){ try{ const v=await src(); if(v){ cache.price[key]={v,ts:now}; return v; } }catch(e){continue;} }
    return cache.price[key]?.v||null;
  }

  // Forex pairs — try multiple sources
  ...
  const sources=[
    // Stooq CSV
    async()=>{ const r=await fetch(`https://stooq.com/q/l/?s=${stooqSym}&f=sd2t2ohlcv&h&e=csv`,{timeout:6000}); const t=await r.text(); if(t.includes('N/D')||t.includes('apikey')) return null; const p=t.trim().split('\n')[1]?.split(','); const v=parseFloat(p?.[6]); return v>0?v:null; },
    // Frankfurter API
    async()=>{ if(quote!=='USD'&&base!=='USD') return null; const from=base==='USD'?quote:base; const to=base==='USD'?base:quote; const r=await fetch(`https://api.frankfurter.app/latest?from=${from}&to=${to}`,{timeout:6000}); const d=await r.json(); const v=d?.rates?.[to]; if(!v) return null; return base==='USD'?parseFloat((1/v).toFixed(5)):parseFloat(v); },
    // ExchangeRate API
    async()=>{ const r=await fetch(`https://open.er-api.com/v6/latest/${base}`,{timeout:6000}); const d=await r.json(); const v=d?.rates?.[quote]; return v?parseFloat(v):null; },
  ];

  for(const src of sources){
    try{
      const v=await src();
      if(v&&!isNaN(v)&&v>0){ cache.price[key]={v,ts:now}; return v; }
    }catch(e){continue;}
  }
  return cache.price[key]?.v||null;
}
...
function calcLevels(signal,price,atr,symbol,sr){
  ...
  if(signal==='BUY'){
    ...
    tp=parseFloat((price+Math.abs(price-sl)*2.5).toFixed(dp));
    ...
    tp=parseFloat((price-Math.abs(sl-price)*2.5).toFixed(dp));
    if(sl<=price)sl=parseFloat((price+safeAtr*2).toFixed(dp));
    if(tp>=price)tp=parseFloat((price-safeAtr*5).toFixed(dp));
  }else{
  ...
  return{sl:sl.toFixed(dp),tp:tp.toFixed(dp),rr};
}
...
function calcRisk(price,sl,symbol,balance=1000,riskPct=1){
  ...
  const slPips=Math.abs(price-parseFloat(sl))/pipVal;
  const pplot=symbol==='USD/JPY'?9.3:10;
  const lots=slPips>0?riskAmt/(slPips*pplot):0.01;
  return{riskAmount:riskAmt.toFixed(2),slPips:slPips.toFixed(1),suggestedLot:Math.min(Math.max(parseFloat(lots.toFixed(2)),0.01),10)};
}
...
function deepSignalEngine(ind,mtfBias,dxy,symbol){
  ...
  if(session.killzone){
    if(bull>bear){bull+=2;factors.push(`✅ ${session.name} KILLZONE active`);}
    else if(bear>bull){bear+=2;factors.push(`✅ ${session.name} KILLZONE active`);}
  }
...
async function getSignalForPair(pair){
  ...
  let signalCandles=h4||daily||buildSyntheticCandles(livePrice,pair.symbol);
  if(!signalCandles||signalCandles.length<15) signalCandles=buildSyntheticCandles(livePrice,pair.symbol);
  ...
  const ind={
    ...
    roundNumber:checkRoundNumbers(livePrice,pair.symbol),
  ...
  const levels=calcLevels(result.signal,livePrice,atr,pair.symbol,ind.sr);
  const entryZone=calcEntryZone(result.signal,livePrice,atr||0.001,pair.symbol);
  ...
  const data={
    symbol:pair.symbol,price:livePrice.toFixed(pair.dp),
    signal:result.signal,phase:result.phase,confidence:result.confidence,
    entry:livePrice.toFixed(pair.dp),sl:levels.sl,tp:levels.tp,rr:levels.rr,
    entryZone,
...
async function getFCMAccessToken() {
  ...
  try {
    ...
    if (!privateKey || !privateKey.includes('BEGIN PRIVATE KEY')) {
      console.log('⚠️ No Firebase key — push disabled');
      return null;
    }

    // Sign JWT with private key using Node crypto
...
app.get('/api/news',async(req,res)=>{
  ...
  try{
    const url=`https://newsapi.org/v2/everything?q=forex+dollar+ECB+Fed+interest+rates+gold+DXY+inflation&language=en&sortBy=publishedAt&pageSize=15&apiKey=${NEWS_KEY}`;
...
app.listen(PORT,()=>console.log(`✅ TRADEPLUS v12 Professional Signal Engine on port ${PORT}`));
