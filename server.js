...
async function getStooqPrice(stooqSym) {
  ...
  if (stooqSym.includes('xau')) {
    ...
    const goldSources = [
      'https://stooq.com/q/l/?s=xauusd.cf&f=sd2t2ohlcv&h&e=csv',
      'https://stooq.com/q/l/?s=xauusd&f=sd2t2ohlcv&h&e=csv',
      'https://api.metals.live/v1/spot/gold',
    ];
    for (const src of goldSources) {
      try {
        const res = await fetch(src, {timeout:6000});
        const text = await res.text();
        let price;
        if (src.includes('metals.live')) {
          const j = JSON.parse(text);
          price = j.price || j[0]?.price;
        } else {
          const lines = text.trim().split('\n');
          if (lines.length < 2) continue;
          price = parseFloat(lines[1].split(',')[6]);
        }
  ...
  try {
    ...
    const text = await res.text();
    const lines = text.trim().split('\n');
    if (lines.length < 2) return cache.price[key]?.v||null;
